...
import type { ChartDataSeries, ChartLineShape } from '../chart';
...
export const usePointMarkStore = create<PointMarkState & PointMarkActions>((set, get) => ({
  clicksByChart: {},
  ...
  addPointMark: (chartId, xValue, yValue, chartData, options) => {
    ...
    if (clicks.length === 3) {
      ...
      const bindableIndicesBase =
        seriesBindable != null
          ? Array.from({ length: seriesCount }, (_, i) => i).filter(
              (i) => seriesBindable![i] !== false
            )
  ...
  addShapes: (chartId, shapes) =>
    set((s) => ({
      shapesByChart: {
        ...s.shapesByChart,
        [chartId]: [...(s.shapesByChart[chartId] ?? []), ...shapes],
      },
    })),
